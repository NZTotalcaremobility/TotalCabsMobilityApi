...
const { isMatch, isNull } = require("lodash");
...
module.exports = {
  ...
  customerMobileSignup: customerMobileSignup,
  ...
  adminGetJobByid:adminGetJobByid,
...
async function customerMobileSignup(req, res) {
  console.log('req-------', req);
  let data = {
  ...
  let saveobj = new customermodel(data);
  await saveobj.save();
  utility.sucesshandler(res, 'Registered successfully.');
}
...
async function customerSignupOtp(req, res) {
  customermodel.existPhonenumberCheck(req.body.phonenumber, async function (err, exist) {
    if (err) {
      ...
      if(finalData){
      utility.sucesshandler(res, constantmessage.messages.otpsend, finalData);
      }
      else{
        utility.errorhandler(res,constantmessage.messages.Invalidotp)
      }
    }
  });
}
...
function editDriver(req, res) {
  ...
  async function update_account() {
    try {
      ...
      let customerObj = await customermodel.findByIdAndUpdate( condition, dataObj);
...
function adminGetCustomerByid(req, res) {
  async function get_customerByid() {
    try {
      console.log("adin body--",req.body);
      // let condition = {
      //   _id: (req.body.id,
      // };
      let customerdata = await customermodel.findOne({_id:req.body._id});
      console.log("data--",customerdata);
      if (customerdata) {
...
function adminGetJobByid(req, res) {
  async function get_jobByid() {
    try {
      console.log("adin body--",req.body);
      // let condition = {
      //   _id: (req.body.id,
      // };
      let jobmodel = await driverstatusmodel.findOne({_id:req.body.id}).populate(' user ').sort({createdAt:1})
      var user = jobmodel.user
      let customerdata = await notificationmodel.findOne({from:user})
      console.log("data--",jobmodel);
      let data ={
        jobdetail: jobmodel,
  ...
  async function asy_init() {
    try {
      ...
      let customerlist = await customermodel
        .find({userType: "Normal","companyname":{$eq:null},"isDeleted":{$ne:true}})
        .sort({ createdAt: -1 })
        .lean();
...
async function adminDeleteUser(req, res) {
  ...
  await customermodel
    .findByIdAndUpdate(condition, {isDeleted: true})
    .exec((err, resp) => {
      if (err) {
        return res.json({
          status: 404,
...
function adminGetDriver(req, res) {
  async function asy_init() {
    try {
      ...
      let customerlist = await customermodel
        .find({userType: "Driver","isDeleted":{$ne:true}})
        .sort({ createdAt: -1 });
      // console.log("feed--", customerlist);
      if (customerlist) {
        utility.sucesshandler(
...
function adminUpdateDriver(req, res) {
  ...
  async function update_customer() {
    try {
      ...
      let customerObj = await customermodel.findByIdAndUpdate(condition,
        dataObj)
