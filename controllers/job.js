...
function notifyMobile(req,res) {
  
    console.log("in notify mobile");
      try {
        let data ={
          message:"no Driver Accept Your Request"
        }
        return res.json({
          code: 400,
          message: "no Driver Accept Your Request",
...
async function riderequest(req, res) {
  try {
    ...
    console.log(" riderequet body---", body);
    carType = body.taxiType
    console.log("taxi",carType);
    ...
    let availabledrivers = await userModel
      ...
      .select({ _id: 1 });
    console.log("avlaaaa driver-----", availabledrivers);
    const totalDriver = availabledrivers.length;
    console.log("totlal length of driver----",totalDriver);
    notifiedUser = availabledrivers
    let driverstatus = await jobModel.findOneAndUpdate({user:body.riderid},{$set:{notifiedUser:availabledrivers}}).sort({_id:-1})
    ...
    if (availabledrivers.length > 0) {
      ...
      console.log("latest job--###########################################", getLatestjob.jobid._id);
      ...
      async.eachSeries(
          ...
          userModel
            ...
            .exec((err, result) => {
              ...
              if (result.isAvailable == true) {
                ...
                if (deviceType == "Android") {
                  ...
                  var message = {
                    ...
                    data: {
                      ...
                      dateOfJourney: getLatestjob.jobid.dateOfJourney,
                      ...
                      jobid: jobid,
                      pickuptime : getLatestjob.jobid.pickUptime,

                      fare: amount,
                    },
                  };
                  console.log(" Android mess--", message);
                  ...
                  console.log("ios mesage --", message);
                  fcm.send(message, function (err, pushresponse) {
                    if (err) {
                      console.log("notifications not send", err);
                    } else {
                ...
                var ab = totalDriver * 34000;
                
                console.log("ab Time----",ab);
                let calcTime = 0;
                calcTime  =  calcTime + 34000;
                  setTimeout(async () => {
                  ...
                  if (checkrequest.requestAction == "Accepted" ) {
                    ...
                    //  console.log("in calc---");
                    notifyMobile(req,res);
                  
                    //  res = ({
                    //   code: 200,
                    //   message: "No Driver Accept Your Accept",
                    //   data: [],
                    // });
                
                    //callback("No Driver Accept your Request1");
                   }
                   else {
                    const log ={
                      dt : new Date(),
                 ...
                 notifyMobile(req,res);
                  ...
                  // });
                    ...
                    //callback("No Driver Accept your Request12");
                  }
                  calcTime  =  calcTime + 34000;
                  console.log("calct---",calcTime);
                }, 34000);
        ...
        async (err, result) => {
          if (err) {
            console.log("error file in 495",err);
          } else {
  ...
  try {
    ...
    if (body.ridestatus == "Completed") {
      ...
      let tripdetails = await transactionmodel
        ...
        .populate(
          "jobid",
          "pickupLocation dropLocation status jobtype tripstatus pickupTime tripStartTime tripendtime"
        )
      ...
      tripdetails.forEach((element) => {
        ...
        if (element.jobid.jobtype == "Coverjob" || element.jobid.jobtype == "DispatchJob" || element.jobid.jobtype == "Normal") {
          console.log("in hail"+element);
          ...
          let obj = {
            ...
            rating: 0,
            trackid : element.jobid.jobid,
            username: element.user.name ? element.user.name : "",
            userid : element.user._id ? element.user._id :"",
            image: element.user.imagefile ? element.user.imagefile : "",
            starttime: element.jobid.status[1].triptime ? element.jobid.status[1].triptime :null,
            endtime:element.jobid.status[2].triptime ? element.jobid.status[2].triptime :null ,
          ...
          console.log("res--", completedresult);
           ...
           console.log("elem--",element.jobid)
          let obj = {
            ...
            starttime:element.jobid.tripStartTime ? element.jobid.tripStartTime : null,
            endtime:element.jobid.tripendtime ?element.jobid.tripendtime :null ,
            // tripdetails: tripdetails
          };
          completedresult.push(obj);
        }
...
function coverJob(req, res) {
  async function cover_job() {
    ...
    foundedUser  = await userModel.findOne(
             ...
             {name:req.body.user.name},{userType:"Normal"}
      ...
      if (transectionObj) {
        ...
        if (!req.body.mailby || (req.body.mailby && req.body.mailby == 'Email')) {
          ...
          let contentBody = `Hi ${userDetails.name},
                              ...
                              You can see the route and track of the journey from the link given below: 
...
async function coverJobStart(req, res) {
  ...
  try {
    ...
    let jobrequetUpdate = await jobmodel.findOneAndUpdate(
      ...
      console.log("data--",jobrequetUpdate);
    if (jobrequetUpdate) {
            ...
            .exec((err, result) => {
              ...
              if (result.isAvailable == true) {
                ...
                setTimeout(async () => {
                  ...
                  console.log("checkUser---++++",checkrequest);
...
function adminAddDespatchJobs(req, res) {
  ...
  //let existingDriver = await jobModel.findOne()
      ...
      dataObj.requestAction = "Accepted";
      dataObj.distance = req.body.totaldistance;
      dataObj.duration = req.body.duration;
      ...
      dataObj.midlocation = midlocations;
      dataObj.dayOfJourney = dayOfJourney;
      console.log("body",req.body.driverdetails);
      if(req.body.driverdetails == undefined || req.body.driverdetails == null){
      dataObj.driverdetails =null
      }
      else{
      ...
      if (data) {
       ...
       if(DriverData !=null){
        driverlicencsenumber = DriverData.licencenumber
       }
        var userDetails = {
          email: customerdata.email,
          name: customerdata.name,
          ...
          licencenumber: driverlicencsenumber,
        ...
        console.log("bookingd--", dataObj.pickUptime);
        if(driverDetails){
        ...
        utility.sucesshandler(res, 'Add  job successfully', despatchObj);
      }
      }
    }
...
function updatecoverJob(req, res) {
  async function cover_job() {
    try {
      ...
      tObj.tax = req.body.tax || 3;
      tObj.fare = req.body.price ? req.body.price : (Number(req.body.price) - 3).toFixed(2);
      tObj.Amount = req.body.price;
      tObj.type = req.body.paymentType;
     ...
     // tObj.user = mongoose.Types.ObjectId(userID);
      ...
      if (transectionObj) {
        ...
        const curentBooking = await jobmodel.findOne({ user: mongoose.Types.ObjectId(userID) }).populate('driverdetails').sort({ _id: -1 })
        const userDetails1 = await userModel.findOne({ _id: mongoose.Types.ObjectId(userID) });
         // const curentBooking = await jobmodel.findOne({ user: mongoose.Types.ObjectId(userID),jobtype:{$eq:"DispatchJob" }}).sort({ _id: -1 })

        //const driverDetails = await userModel.findOne({ userType: 'Driver', _id: mongoose.Types.ObjectId(req.body.driverdetails) });
        console.log("cover bookingd--", curentBooking.driverdetails);
        if(curentBooking.driverdetails !=null)
        {
          console.log("in  2686");
          let arr = curentBooking.driverdetails.deviceInfo;
          let deviceToken = arr[arr.length - 1].deviceToken;
          let deviceType = arr[arr.length - 1].deviceType;
          ...
          emailtemplate
            ...
            .exec(async (err, emaildataresult) => {
              ...
              if (err) {
                ...
                utility.sucesshandler(res, ' Cover job updated successfully', customerdata);
              }
            });
        }
        else if (req.body.mailby == 'Phone') {
...
function updateDispatchJob(req, res) {
  ...
  action : "",
    ...
    // }
      ...
      dataObj.driverdetails = mongoose.Types.ObjectId(req.body.driverdetails);
      dataObj.fare = req.body.price;
      transObj.fare = req.body.price ? req.body.price : (Number(req.body.price) - 3).toFixed(2);
      transObj.Amount = req.body.price;
      transObj.tax = 3;
    console.log("currentjob id--",req.body._id);
    let customerObj = await jobmodel.findOneAndUpdate(({_id:req.body._id}) ,dataObj );

    // tObj.jobid = mongoose.Types.ObjectId(customerObj.userData._id);
    // tObj.user = mongoose.Types.ObjectId(userID);
     console.log("customerr---", customerObj);
     let transectionObj = await transectionModel( ({jobid:req.body._id}),transObj)
      let existDriver = await jobmodel.findOne({_id:req.body._id},select = 'driverdetails').populate('driverdetails')
      ...
      if (result.status) {
        ...
        if (data) {
         ...
         console.log("olddriverid--,",oldDriver);
          ...
          if(existDriver && existDriver.driverdetails){
            console.log("in exist driver-------------------2794");
            var oldDriver = existDriver.driverdetails._id
          var oldriverinfo = existDriver.driverdetails.deviceInfo
          ...
          if (data.mailby == 'Email') {
            ...
            utility.sucesshandler(res, ' dispatch job successfully Updated');
...
async function adminDeleteDispatchJob(req, res) {
  ...
  try {
    ...
    let transcetiodel = await transactionmodel.findOneAndRemove({jobid:req.body._id})
    console.log({transcetiodel});
    if(transcetiodel){
      console.log("in ob");
      let jobmodel = await jobModel.findOneAndRemove({_id:req.body._id})
      console.log({jobmodel});
      if(jobmodel){
        console.log("jobmodel");
         utility.sucesshandler(res,"Dispatch Job deleted successfully",jobmodel)
      }
      else{
        utility.errorhandler("issue in del job")
      }
     
    }
    else{
      utility.errorhandler(res,"job is not deleted")
    }
    // await transactionmodel.findByIdAndRemove(condition).exec((err, resp) => {
    //   if (err) {
    //     return res.json({
    //       status: 404,
    ...
    //   else {
    //     console.log("in else");
    //    jobmodel.findByIdAndRemove({_id:req.body._id}).exec((err,resp)=>{
    //       if (err) {
    //         return res.json({
    //           status: 404,
    //           msg: "somthing went wrong !",
