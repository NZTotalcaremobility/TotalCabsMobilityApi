...
var serverkey = "AAAAe2dFb3k:APA91bH1sVSzIm2RcC3TehkXMxTrzlJjATuHvCO4VsM2CyU8azuF_F6n89I9OlAKzRKI15TmlElBIjfznAma4OtlrsJzf3Hs_fSRHvrK7YAyYf2m3R-orykYtD28HnXDaCOQKByJKdLZ";
...
function showAvailableVehicle(req, res) {
  async function _showAvailableVehicle() {
    try {
      ...
      if (result.data && result.data.rows[0].elements && result.data && result.data.rows[0].elements[0].status === 'OK') {
        totalResult = {
      ...
      const availableVehicles = await queryy.findData(customermodel, { userType: "Driver", onlinestatus: 1, carType: carType }, "name carType carBrand carRegNo currentLocation", null)

      if (availableVehicles.error) throw { messgae: "Internal Server Error!" }
      if (!availableVehicles.status || _.isEmpty(availableVehicles.data)) utility.sucesshandler(res, "No Data Found", [])
      ...
      data.forEach(async (element, index) => {
        ...
        if (currentLocation && currentLocation.coordinates) {
          destinationArry += `${currentLocation.coordinates.reverse().toString()}|`
      ...
      if (destinationArry) {
        ...
        axios.get(link)
          .then(function (response) {
            if (response.data && !_.isEmpty(response.data.rows) && response.data.rows[0].elements)
              response.data.rows[0].elements.forEach((row, index) => {
                const { status } = row;
                if (status === 'OK') {
                  const { distance, duration } = row
                  if (responseResult[finalResult[index].carType]) {
                    if (responseResult[finalResult[index].carType].distance > distance.value) {
                      responseResult[finalResult[index].carType] = { duration: duration.text, distance: distance.value, ...finalResult[index] }
                    }
                  } else {
                    responseResult[finalResult[index].carType] = { distance: distance.value, duration: duration.text, ...finalResult[index] }
                  }
                }
              });
            let okResult = {
...
async function customersignup(req, res) {
  try {
    ...
    if (body.email) {
      ...
      customermodel.existPhonenumberCheck(body.phonenumber, async function (err, exist) {
        console.log('e', exist)
        if (err) {
          throw err
        } else if (exist !== true) {
                ...
                .exec(async (err, emaildataresult) => {
                  if (err) {
                    ...
                    if (userType === 'Admin' || userType === 'SubAdmin') {
                      reset_urlToken = constantmessage.adminURL.url +
                        "/resetPassword/" +
                        userMailData.randomToken;

                    } else {
                      reset_urlToken = constantmessage.weburl.url +
                        "/auth/reset-password/" +
                        userMailData.randomToken;
                    }
                    // let reset_link =
  ...
  try {
    ...
    if (!userresult) {
      ...
      if (!userresult) {
        ...
        let savedata = await userresult.save();
        if (savedata) {
          utility.sucesshandler(res, "Verified Sucsessfully", { token: randomToken });
        }
      }
    } else {
      let randomToken = randomstring.generate(30);
              ...
              // }
                ...
                userInfo.comparePassword(body.password, async function (
                  ...
                  if (isMatch && !err) {
                    ...
                    let update = await queryy.updateOneDocument(customermodel, condition, dataObj);
                    ...
                    infoData.isEmailVerified = userInfo.isEmailVerified ? userInfo.isEmailVerified : false;
                    infoData.isPhoneVerified = userInfo.phoneverified ? userInfo.phoneverified : false;
...
async function otpverification(req, res) {
  try {
    ...
    var code = await customermodel.findOne({ otp: password, phonenumber: phonenumber });
    if (code) {
...
function adminGetCustomer(req, res) {
  async function asy_init() {
    try {
      ...
      let customerlist = await customermodel.find(condition).sort({ createdAt: -1 }).lean();

      if (customerlist) {
        customerlist = customerlist.map((data) => (!data.keyStatus ? _.omit(data, ['key']) : data))
        utility.sucesshandler(res, constantmessage.messages.userDataFetched, customerlist);
...
function adminUpdateCustomer(req, res) {
  ...
  async function update_customer() {
    try {
      ...
      let customerObj = await queryy.updateOneDocument(Customer, condition, dataObj);

      if (customerObj.status) {
        utility.sucesshandler(res, constantmessage.messages.customerDataUpdated, customerObj);
...
async function adminDeleteUser(req, res) {
  ...
  await customermodel.findByIdAndRemove(condition, { isDeleted: body.isDeleted }).exec((err, resp) => {
    if (err) {
      return res.json({
        status: 404,
...
function adminGetDriver(req, res) {
  async function asy_init() {
    try {
      ...
      let customerlist = await customermodel.find(condition).sort({ createdAt: -1 });
      // console.log("feed--", customerlist);
      if (customerlist) {
...
function adminGetDriverByIdWithReview(req, res) {
  ...
  async function get_driverByid() {
    try {
      ...
      if (customerdata.status) {
        utility.sucesshandler(res, constantmessage.messages.customerDataFetched, obj);
      } else {
        utility.sucesshandler(res, constantmessage.validationMessages.Invaliddata);
      }
    } catch (e) {
...
function adminUpdateDriver(req, res) {
  ...
  async function update_customer() {
    try {
      ...
      if (customerObj.status) {
        utility.sucesshandler(res, constantmessage.messages.driverDataUpdated, customerObj);
...
function searchUser(req, res) {
  const _searchUser = async () => {
    ...
    try {
      let customerlist = await queryy.findData(customermodel, condition, fetachvalue, sort, null);
    ...
    try {
      ...
      if (customerdata.status) {
        if (customerdata.data.isBlock == true) {
          ...
          let customerObj = await queryy.updateOneDocument(Customer, condition, obj);
          if (customerObj.status) {
            utility.sucesshandler(res, constantmessage.messages.customerDataFetched, customerObj);
  ...
  async function _updateKeyStatus() {
    try {
      ...
      if (customerObj.status) {
        ...
        utility.sucesshandler(
          res,
          req.body.keyStatus ? "Key successfully enabled!" : "Key successfully disabled!",
        );
      } else {
        utility.sucesshandler(
          res,
